...
class SQLManager(object):
	...
	# 查询多条数据
	def get_list(self, sql:str, args:tuple=None):
		self.cursor.execute(sql, args)
		result = self.cursor.fetchall()
		return result

	# 查询单条数据
	def get_one(self, sql:str, args:tuple=None):
		self.cursor.execute(sql, args)
		result = self.cursor.fetchone()
		return result

	# 执行单条SQL语句
	def moddify(self, sql:str, args:tuple=None):
		self.cursor.execute(sql, args)
		self.conn.commit()
	...
	# 方法就可以了。
	# 执行多条SQL语句
	def multi_modify(self, sql:str, args:list=None):
